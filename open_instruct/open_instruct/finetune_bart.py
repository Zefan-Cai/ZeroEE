...
def encode_with_prompt_completion_format(example, tokenizer, max_seq_length):
    ...
    targets = tokenizer(example['completion'], return_tensors='pt', padding=True, max_length=16, truncation=True)
    ...
    dec_idxs = torch.cat((dec_idxs, torch.tensor([0] * (16 - dec_idxs.size(dim=0)))), dim=0)
    dec_attn = torch.cat((dec_attn, torch.tensor([0] * (16 - dec_attn.size(dim=0)))), dim=0)
    ...
    # print('enc_attn', enc_attn.size())
    # print('dec_idxs', dec_idxs.size())
    # print('dec_attn', dec_attn.size())
    # print('lbl_idxs', lbl_idxs.size())

    return {
        'input_ids': enc_idxs.flatten(),
        'attention_mask': enc_attn.flatten(),
        'decoder_input_ids': dec_idxs.flatten(),
        'decoder_attention_mask': dec_attn.flatten(),
        'labels': lbl_idxs.flatten()
    }
