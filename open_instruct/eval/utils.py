...
import asyncio
import os
from transformers import StoppingCriteria, AutoModelForPreTraining
...
def load_hf_lm_and_tokenizer_bart(
        model_name_or_path, 
        tokenizer_name_or_path=None, 
        device_map="auto", 
        load_in_8bit=False, 
...
@torch.no_grad()
def generate_completions_bart(model, tokenizer, prompts, batch_size=1, stop_id_sequences=None, disable_tqdm=False, **generation_kwargs):
    generations = []
    if not disable_tqdm:
        progress = tqdm.tqdm(total=len(prompts), desc="Generating Completions")
