...
class Data():
    ...
    def get_valid_data(self):
        ...
        for split in range(1, 6):
            with open(os.path.join(self.args.base_dir, self.args.data_dir, self.args.dataset_name.lower(), f"split{str(split)}", 'dev.json'), 'r') as fp:
            ...
            for index in range(len(ACE_valid_data)):
                ...
                for event_index in range(len(ACE_valid_data[index]["event_mentions"])):
                    event_type = ACE_valid_data[index]["event_mentions"][event_index]["event_type"]
                    trigger = ACE_valid_data[index]["event_mentions"][event_index]["trigger"]["text"]
                ...
                for event_type in self.event_type2definition.keys():
                    ...
                    if self.args.dataset_name == "ACE":
                        display_event_name = event_type.split(':')[1]
                    elif self.args.dataset_name == "PHEE":
                        display_event_name = event_type.split('_event')[0]
                    elif self.args.dataset_name == "MEE":
                        display_event_name = event_type.split('_')[1]
                    elif self.args.dataset_name == "M2E2":
                        display_event_name = event_type.split(':')[1]
                    elif self.args.dataset_name == "CASIE":
                        display_event_name = event_type.split(':')[1]

                    if self.args.template_version == "v1":
                        prompt = f"{sample} \n The event is: {display_event_name}. \n The event definition is: {event_definition} \n The parent event is {parent_event}, son events include {text_sons}. \n So what is the trigger? Event trigger is "
                        if self.args.setting == "inference":
                            prompt = f"{sample} \n The event is: {display_event_name}. \n The event definition is: {event_definition} \n The parent event is {parent_event}, son events include {text_sons}. \n So what is the trigger? Event trigger is "
                    elif self.args.template_version == "v2":
                        prompt = f"SENTENCE: {sample} \n EVENT TYPE: {display_event_name}. \n DEFINITION: {event_definition} \n PARENT: {parent_event}, SON: {text_sons}. \n So what is the trigger? Event trigger is "
                        if self.args.setting == "inference":
                            prompt = f"SENTENCE: {sample} \n EVENT TYPE: {display_event_name}. \n DEFINITION: {event_definition} \n PARENT: {parent_event}, SON: {text_sons}. \n So what is the trigger? Event trigger is "
    ...
    def get_test_data(self):
        ...
        ACE_test_data = []
        for split in range(1, 6):
            with open(os.path.join(self.args.base_dir, self.args.data_dir, self.args.dataset_name.lower(), f"split{str(split)}", 'test.json'), 'r') as fp:
                for line in fp.readlines():
            ...
            for index in range(len(ACE_test_data)):
                ...
                for event_index in range(len(ACE_test_data[index]["event_mentions"])):
                    event_type = ACE_test_data[index]["event_mentions"][event_index]["event_type"]
                    trigger = ACE_test_data[index]["event_mentions"][event_index]["trigger"]["text"]
...
def main():
    ...
    parser.add_argument('--data_dir', default='/data/ace_v2', type=str, help='dir to generated data')
    ...
    parser.add_argument('--dataset_name', default='ACE', type=str, help='dataset filename')
